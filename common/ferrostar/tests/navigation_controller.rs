...
use ferrostar::models::{GeographicCoordinate, Route, UserLocation};
...
#[test]
fn kan_69_active_serializable_state_round_trips_json() {
    use std::time::{Duration, UNIX_EPOCH};

    let value = SerializableStepAdvanceCondition::DistanceEntryAndExitWithUTurnConfirmation {
        ...
        movement_anchor: Some(UserLocation {
            ...
            timestamp: UNIX_EPOCH + Duration::from_secs(1_785_319_202),
            speed: None,
        }),
        confirmation_count: 1,
        last_evaluated_timestamp: Some(UNIX_EPOCH + Duration::from_secs(1_785_319_202)),
    };

    let json = serde_json::to_string(&value).unwrap();
    let decoded: SerializableStepAdvanceCondition = serde_json::from_str(&json).unwrap();
    let restored: Arc<dyn StepAdvanceCondition> = decoded.into();
